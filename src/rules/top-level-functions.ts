...
const topLevelFunctions = ESLintUtils.RuleCreator(() => 'https://github.com/tomerh2001/eslint-plugin-th-rules/blob/main/docs/rules/top-level-functions.md')({
	...
	create(context) {
		...
		function buildArrowFunctionReplacement(functionName: string, arrow: TSESTree.ArrowFunctionExpression, isExport: boolean): string {
		...
		function buildFunctionExpressionReplacement(functionName: string, funcExpr: TSESTree.FunctionExpression, isExport: boolean): string {
		...
		function buildAnonymousFunctionDeclarationReplacement(node: TSESTree.FunctionDeclaration, functionName = 'defaultFunction', isExport = false): string {
