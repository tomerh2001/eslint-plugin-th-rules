...
import rule, { PREDICATE_METHOD_NAMES, ITERATEE_METHOD_NAMES } from '../../src/rules/prefer-lodash-iteratee-shorthand';
...
function buildIterateeSingleFunctionCases() {
	const valid: string[] = [];
	const invalid: Case[] = [];
...
const predicateCases = buildPredicateCases();
const iterateeCases = buildIterateeSingleFunctionCases();
const sortOrderCases = buildSortOrderArrayIterateeCases();

ruleTester.run('prefer-lodash-iteratee-shorthand', rule, {
	valid: [
		...predicateCases.valid,
		...iterateeCases.valid,
	...
	invalid: [
		...predicateCases.invalid,
		...iterateeCases.invalid,
		...sortOrderCases.invalid,

		{
			code: 'collection.find((x) => x.Y === z);',
			output: "import _ from 'lodash';\n_.find(collection, {Y: z});",
			errors: error('useLodashFind'),
		},
		{
			code: 'collection.find(function (x) { return x.Y === z; });',
			output: "import _ from 'lodash';\n_.find(collection, {Y: z});",
			errors: error('useLodashFind'),
		},
		{
